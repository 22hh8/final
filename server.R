...
shinyServer(function(input,output,session) {
    ...
    output$tate <- DT::
        tate %>% filter(acquisitionYear <= input$date)
    })
    
    output$gender <- renderPlot({
        tate1 <- tate %>% filter(acquisitionYear <= input$date, !is.na(gender)) %>% 
    ...
    output$medium <- renderPlot({
        medium <- tate %>% filter(acquisitionYear <= input$date, !is.na(medium)) %>% group_by(medium) %>% count() %>% arrange(desc(n)) %>% head(5)
    ...
    output$inscription <- renderPlot({
        tate1 <- tate %>% filter(acquisitionYear <= input$date) %>% group_by(inscription) %>% count()
    ...
    output$year <- renderPlot({
        tate1 <- tate %>% filter(acquisitionYear <= input$date) %>% select(year) %>% 
    ...
    output$artist <- renderValueBox({
        topartist <- tate %>% filter(acquisitionYear <= input$date, !is.na(name)) %>% group_by(name) %>% count() %>% arrange(desc(n)) %>% head(1)
        valueBox(
    ...
    output$total <- renderValueBox({
        total <- tate %>% filter(acquisitionYear <= input$date) %>% count()
        valueBox(
